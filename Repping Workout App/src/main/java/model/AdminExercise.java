...
public class AdminExercise implements Serializable {
    ...
    public static void setExerciseList(int userid) {
        Connection connection = null;
        ResultSet resultSet = null;
        PreparedStatement preparedStatement = null;
        try {
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/reppingdatabase", "root", "p2590746");
            preparedStatement = connection.prepareStatement("SELECT DISTINCT ExerciseName FROM tblexercise WHERE UserIdentification IS NULL OR UserIdentification = " + userid);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
    ...
    public static void setExerciseID(String ExerciseName, int userid) {
        Connection connection = null;
        ResultSet resultSet = null;
        PreparedStatement preparedStatement = null;
        try {
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/reppingdatabase", "root", "p2590746");
            preparedStatement = connection.prepareStatement("SELECT ExerciseID FROM tblexercise WHERE ExerciseName = '" + ExerciseName + "' AND UserIdentification = " + userid);
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
